...
abstract class AbstractPdf extends Magento\Sales\Model\Order\Pdf\AbstractPdf
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if (!$order->getIsVirtual()) {
            $page->drawText(__('Ship to:'), $Address_x, $top - 15, 'UTF-8');
        } else {
            $page->drawText(__('Payment Method:'), $Address_x, $top - 15, 'UTF-8');
        }
        ...
        if (!$order->getIsVirtual()) {
            ...
            foreach ($shippingAddress as $value) {
                if ($value !== '') {
                    ...
                    foreach ($text as $part) {
                        $page->drawText(strip_tags(ltrim($part ?: '')), $Address_x, $this->y, 'UTF-8');
                        $this->y -= 15;
                    }
                }
            }
            ...
            $page->drawText(__('Shipping Method:'), $Address_x, $this->y, 'UTF-8');
        ...
        if ($order->getIsVirtual()) {
            ...
            if (isset($shippingMethod) && \is_string($shippingMethod)) {
                foreach ($this->string->split($shippingMethod, 45, true, true) as $_value) {
                    $page->drawText(strip_tags(trim($_value ?: '')), $Address_x, $this->y, 'UTF-8');
                    $this->y -= 15;
                }
            }
            ...
            $page->drawText($totalShippingChargesText, $Address_x, $yShipments - $topMargin, 'UTF-8');
            ...
            if (count($tracks)) {
                $page->setFillColor(new \Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
                $page->setLineWidth(0.5);
                $page->drawRectangle($Address_x, $yShipments, 510, $yShipments - 10);
